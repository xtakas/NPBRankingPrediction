...
- 的中セルは、まだ順位が変わる可能性がある間は緑、残り試合数から見てもう変わり得ないと判定できたら金色で表示します(下記「順位確定判定」参照)。
...
各球団の「現在の順位」が、残り試合の結果によらずもう変わり得ないかどうかを `StandingsCalculator`(スクレイピング時)で判定し、`standings.json` の各チームに `isRankConfirmed` として保持しています。Web側はこの値を読むだけなので重い計算はブラウザ上では行いません。

- **正確な数学的確定判定ではなく簡易な近似**です。実際の残り対戦カード(誰と何試合残っているか)は取得しておらず、NPBのレギュラーシーズンが143試合制であることだけを前提に「残り試合数 = 143 −(消化試合数)」を計算します。
- 判定は**自分より上の全チーム・下の全チームそれぞれとの2チーム間比較**で行います。「上位チームが残り全敗、下位チームが残り全勝」という最悪ケースでも上位チームの勝率が上回っていれば、その2チーム間の順位は確定とみなし、これを隣接チームだけでなく上下すべてのチームに対して行うことで、2つ以上離れたチームが割り込んで逆転するケースも排除しています。
- 各予想者の的中セルは、的中していてもこの判定が「未確定」の間は緑、「確定」になったら金色で表示されます(リーダーボードの「的中!」やシーズン確定バナーとは別の概念です)。
- レギュラーシーズンの試合数が変わった場合は `StandingsCalculator.RegularSeasonGameCount`(現在143)を修正してください。
...
- `predictions.json` には実名は持たせません(スコア計算に使うのは `central`/`pacific` のみ)。これは**GitHubに必ずpushされるファイル**なので、実名は書かないでください。
